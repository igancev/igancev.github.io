...
i3 подерживает возможности отрисовки окон, управления их
...
После запуска i3 нас спросит о создании конфиг файла в домашней
...
Перед самой командой фигрирует ключевое слово `exec`, т.е. запуск,
...
`compton` - лёгкий композитный менеджер для графического сервера X.
...
Из моментально заментых изменений - это плавность переключения
...
и не должен, и не позицианируется как DE. В нем мы должны настроить все
